...
| `renderImageFigures`  | `function renderImageFigures(body)`       | {string} The same body, with each block image as a `<figure>`.                                                                                 | Rewrite every block image in a body into the figure the website publishes.   |
