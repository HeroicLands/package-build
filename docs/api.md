...
| `buildPages`          | `function buildPages(rawPages, entryId, noteName, options)`                       | {Array<{_id: string, name: string, type: string, title: {show: boolean, level: number}, text: {format: number, content: string}, _key: string}>} The page documents, in order. | Compile split pages into JournalEntryPage documents.                                                                             |
| `buildJournalEntry`   | `buildJournalEntry({ id, name, markdown, leadName, folder, flags, stats, stub })` | {object} The JournalEntry document, keyed for the pack.                                                                                                                        | Assemble one JournalEntry document from a note's converted markdown.                                                             |
