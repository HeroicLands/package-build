...
| `blockDataProperty`         | `blockDataProperty(fm, block, key, defaultValue)` | the value                              | reading a system override, then shared `data`, then a top-level value                         |
...
| `authoredKey`               | `authoredKey(name)`                               | `string`                               | reading a shared source as the key an author writes, without the `data:` container holding it |
...
| Export             | Signature                                                  | Returns                                             | Use it when                                                                                   |
| ------------------ | ---------------------------------------------------------- | --------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `SHORT_BODY_WORDS` | `const SHORT_BODY_WORDS`                                   | `number`                                            | reading the word count below which an unmarked body is asked whether it is a draft            |
| `lintNoteStates`   | `lintNoteStates(contentBase, { records, contentPackage })` | `{findings, counts, byType, oldestDrafts, summary}` | checking a tree's states and counting the ladder, from the corpus the command already derived |
...
| `foreignNode`           | `foreignNode(canonical, entry)`          | `PopulationNode\|null` | reading a fetched index entry into a node, or `null` for one that names neither a place nor an affiliation   |
...
| `mapWorld`            | `mapWorld({ records, foreignIndex, contentBase, base, config })` | `MapWorld`    | the same world out of records and indexes a caller already holds, as the site build does                                 |
...
The geometry of the map from a place and of the chart from one, as arithmetic over the place records: north up, east right, the angle from the bearing and the radius from the days marker on log-spaced rings, hops composed into one bearing as long as none turns back. Nothing here touches a file or GraphViz.
...
| `CHART_HORIZON_DAYS` | `const CHART_HORIZON_DAYS`                 | `number`            | the horizon of the chart from a place when none is asked for, 360 days — the end of the scale                                 |
| `RING_DAYS`          | `const RING_DAYS`                          | `readonly number[]` | the days markers that are rings on the map from a place — every marker up to its horizon                                      |
...
| `rimRadius`          | `rimRadius(horizon?)`                      | `number`            | the radius of the rim, one log step beyond the horizon — the map from a place's by default                                    |
| `ringRadius`         | `ringRadius(days, horizon?)`               | `number`            | the radius of the ring a days value sits on — the rim for none, or for a value beyond the horizon                             |
| `rings`              | `rings(horizon?)`                          | `object[]`          | the rings inside out, each marker up to the horizon with its radius                                                           |
...
| `composeChain`       | `composeChain(hops)`                       | `object\|undefined` | a chain of hops composed into one bearing and one total, each hop within an octant of the running direction, else `undefined` |
...
| `layoutChart`        | `layoutChart(places, centre, { horizon })` | `object`            | laying out the chart from a place: the map from it at a larger horizon, the hops chaining on as long as none turns back       |
...
| `hopOpacity` | `hopOpacity(hop)`                                  | `number` | how faint a place is drawn by its hop count — full to the second hop, a step down each hop beyond, floored       |
| `fromDot`    | `fromDot(layout, places, { scale, title })`        | `string` | the map or the chart from a place, every node pinned for `neato -n2`, the rings drawn faintly with their markers |
...
| `blockSystem`            | `function blockSystem(keyPath)`                       | {string} The game system id, or `note`.                                                                                                                                                                 | Which system a frontmatter key path is written under.                                                 |
...
| `SHIPPED_SYSTEMS`             | `const SHIPPED_SYSTEMS`                  | —                                                                                                                                 | Every system this toolchain ships a map for, by id.                                                                                    |
| `SYSTEM_DOCUMENT_CLASSES`     | `const SYSTEM_DOCUMENT_CLASSES`          | —                                                                                                                                 | The Foundry document classes a system compiles its own data into.                                                                      |
...
| `emitContentIndex`      | `emitContentIndex({ contentBase, outDir, config })`                                                     | {{file: string, notes: number, assets: number, records: number, bytes: number, full: number, draft: number, stub: number}} Where it was written, how many notes and how many assets it holds, how many records that is in all, its size, and the ladder those notes sit on. | Emit this package's content index.                                                                                                                                                                                        |
...
`engine.contentIndex` also re-exports eight names from neighbouring leaf modules, at this same import path:
...
| `isStub`              | `isStub(record)`                 | `boolean`                                                      | telling a stub — a note with an empty body — apart from a note that publishes a page; asked of the absent address, which is the whole signal                                     |
...
| `pageDestination`     | `function pageDestination(page, { bundle })`      | —                                                                                                                                      | Where a page is written, relative to the output root: flat, or as a leaf bundle when it carries a file beside itself. |
...
| `parseImageDirective` | `function parseImageDirective(raw)`             | {{classes: string[], size: string, float: string, problems: string[]}} What was written, and what cannot be honoured.                          | Read the directive on an image.                                              |
...
| `renderFoundryMarkdown`   | `function renderFoundryMarkdown(body)`                                                                      | `string` — HTML with secret sections for GM passages.                                                                                                                                                                                   | Rendering a note body for a Foundry document.                                                                                                                                                        |
...
| Export           | Signature                                                       | Returns                                                                                          | Use it when                                                                                                                |
| ---------------- | --------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------- |
| `docEntryTypes`  | `function docEntryTypes()`                                      | {ReadonlySet<string>} The configured doc-carrying types.                                         | Every content type whose **prose compiles into a JournalEntry of its own**, addressed by `note` with the authored type.    |
...
| Export               | Signature                                                               | Returns                                                                                                                                                                                                                                                                                                                                                    | Use it when                                                                                                                                                                |
| -------------------- | ----------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `resolveItemDocType` | `function resolveItemDocType(qualifier, types)`                         | {string\|null} The underlying document type, or `null` when the qualifier is not a virtual one.                                                                                                                                                                                                                                                            | Reads an accepted `doc<type>` input alias as its authored type.                                                                                                            |
...
| `readWikilink`             | `function readWikilink(inner, context)`                    | Typed target or literal target with diagnostic source.          | Parse a link target using complete builder Address context.                     |
...
| `stubAddressMessage`       | `function stubAddressMessage(target, file)`                | {string} The message, unpunctuated at the end as a finding is.  | What an author linking to a stub is told, naming the file to open.              |
...
| Export                        | Signature                                               | Returns                                                                                                                                                                                                         | Use it when                                                                                                         |
| ----------------------------- | ------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| `systemCompilers`             | `function systemCompilers(passes)`                      | {Readonly<Record<string, Readonly<Record<string, Function>>>>} System id → document class → pass.                                                                                                               | The system-specific compilers, derived from the passes themselves.                                                  |
