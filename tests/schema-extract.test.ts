...
describe("buildSchemaArtifact — sub-schemas built by a function", () => {
    beforeAll(() => {
        write(
            "factory.js",
            `
const fields = foundry.data.fields;
const ABILITIES = ["str", "dex"];
...
function abilityField() {
    return new fields.SchemaField({base: new fields.NumberField({})});
}
...
export const itemModels = {skill: SkillModel};
`,
        );
    });

    it("follows a local factory to the SchemaField it returns", () => {
        // A repeated sub-schema is routinely extracted into a function, and the
        // call is as much a declaration as the constructor it wraps. Reading
        // only the constructor form recorded `skillBase` with no keys beneath
        // it, while real content writes `skillBase.value` — so a correctly
        // authored field would have been reported as undeclared.
        const { documents } = extract("factory.js");
        expect(documents.Item.skill.own).toContain("skillBase");
        expect(documents.Item.skill.own).toContain("skillBase.value");
        expect(documents.Item.skill.own).toContain("skillBase.formula");
    });

    it("records a computed SchemaField without inventing its keys", () => {
        // `Object.fromEntries(ABILITIES.map(…))` is computed, not written out.
        // The field is still recorded — which is what keeps everything beneath
        // it from being called undeclared — but its keys are not guessed at.
        // Claiming keys this cannot actually see would be worse than admitting
        // the gap: the check would then report confidently on a shape it made
        // up.
        const { documents } = extract("factory.js");
        expect(documents.Item.skill.own).toContain("abilities");
        expect(
            documents.Item.skill.own.filter((f: string) =>
                f.startsWith("abilities."),
            ),
        ).toEqual([]);
    });
});
